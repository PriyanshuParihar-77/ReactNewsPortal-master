...
class Politics extends Component {
    ...
    searchUsers = async (text) => {
        ...
        if (text == false) {
            ...
            const res = await axios.get(`http://backendhw8-env.eba-qt8jtpq3.us-east-1.elasticbeanstalk.com/GuardianPol`);
            this.setState({ newstype: res.data.response.results });
            this.setState({ type: 'guardian', text1: false })
            localStorage.setItem('user', JSON.stringify(this.state));
        }
        else if (text == true) {
            const res = await axios.get(`http://backendhw8-env.eba-qt8jtpq3.us-east-1.elasticbeanstalk.com/NytimesPol`);
