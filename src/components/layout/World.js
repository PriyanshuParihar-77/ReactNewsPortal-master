...
class World extends Component {
    ...
    searchUsers = async (text) => {
        ...
        if (text == false) {
            ...
            const res = await axios.get(`http://backendhw8-env.eba-qt8jtpq3.us-east-1.elasticbeanstalk.com/GuardianWor`);
            this.setState({ newstype: res.data.response.results });
            this.setState({ type: 'guardian', text1: false })
            localStorage.setItem('user', JSON.stringify(this.state));
        }
        else if (text == true) {
            const res = await axios.get(`http://backendhw8-env.eba-qt8jtpq3.us-east-1.elasticbeanstalk.com/NytimesWor`);
            ...
            <div>
                <Navbar1 searchUsers={this.searchUsers} text1={this.state.text1} barsearchUsers={this.barsearchUsers} what={this.what} />
                {
                    this.state.loading ? <Spinner /> :
                        this.state.type === 'nytimes' && this.state.what === "" && this.state.text1 == true &&
                ...
                {
                    this.state.loading ? <Spinner /> :
                        this.state.type === 'guardian' && this.state.what === "" && this.state.text1 == false &&
                        this.state.newstype.map((user) => (
                            user.webTitle && user.sectionId && user.webPublicationDate && user.blocks.body && user.blocks
                            && user.blocks.body[0] && user.blocks.body[0].bodyTextSummary && user.blocks.main &&
                            user.blocks.main.elements && user.blocks.main.elements["0"] &&
